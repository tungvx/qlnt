...
def password_change(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return HttpResponseRedirect('/school/success')
...
def teacher_account(request, teacher_id):
    ...
    if request.method == 'POST':
        ...
        message = u'Mật khẩu của giáo viên đã được tạo lại giống tên tài khoản'
    t = loader.get_template(os.path.join('school', 'account.html'))
...
def viewClassDetail(request, class_id, sort_type=0, sort_status=0):
            ...
            if request.POST[u'request_type'] == u'del' and pos > 3:
                ...
                try:
                    data = {'first_name': request.POST['first_name'], 'last_name': request.POST['last_name'],
                            ...
                            'sex': request.POST['sex'], 'birth_place': request.POST['birth_place'].strip(),
    ...
    if int(sort_type) == 1:
        if not int(sort_status):
            student_list = cl.students().order_by('first_name', 'last_name', 'birthday')
        else:
            student_list = cl.students().order_by('-first_name', '-last_name', '-birthday')
    if int(sort_type) == 2:
        if not int(sort_status):
            student_list = cl.students().order_by('birthday')
        else:
            student_list = cl.students().order_by('-birthday')
    if int(sort_type) == 3:
        if not int(sort_status):
            student_list = cl.students().order_by('sex')
        else:
            student_list = cl.students().order_by('-sex')
    if int(sort_type) == 4:
        if not int(sort_status):
            student_list = cl.students().order_by('ban_dk')
        else:
            student_list = cl.students().order_by('-ban_dk')
    if int(sort_type) == 5:
        if not int(sort_status):
            student_list = cl.students().order_by('school_join_date')
        else:
            student_list = cl.students().order_by('-school_join_date')
        ...
        if request.method == 'POST' and request.POST['request_type'] == u'rename':
            ...
            if addform.is_valid():
                ...
                for a in addform:
                    if a.name == 'first_name':
                        ...
                        if a.errors:
                            birthday = str(a.errors)
                message = u'Thông tin nhập vào không hợp lệ'
                data = simplejson.dumps({'message': message, 'status': 'failed',
            ...
            if add_group_form.is_valid():
                try:
                    ...
                    message = u'Nhóm này đã tồn tại trong hệ thống'
                    data = simplejson.dumps({'message': message, 'status': 'dup'})
                    return HttpResponse(data, mimetype='json')
                except Exception as e :
                    message = u'Bạn vừa thêm nhóm mới'
                    add_group_form.save()
                    data = simplejson.dumps({'message': message, 'status': 'success'})
                    return HttpResponse(data, mimetype='json')
            else:
...
def subjectAgenda(request, subject_id):
    ...
    if request.is_ajax():
        ...
        elif request.POST['request_type'] == 'note':
            _id = int(request.POST['id'])
            less = sub.lesson_set.get(id=_id)
            less.note = request.POST['value']
            less.save()
            message = u'Đã lưu'
            data = simplejson.dumps({'message': message})
            return HttpResponse(data, mimetype='json')
    ...
    for r in range(start_row+1, sheet.nrows):
        ...
        try:
            try:
                ...
                print ie
                message += u'<ul>STT trống hoặc không đúng ở ô (' + str(r) + ', 1).</ul>'
                continue
            ...
            try:
                _year, _month, _day, _hour, _minute, _second = xlrd.xldate_as_tuple(sheet.cell_value(r, 3),
                                                                                    book.datemode)
                day = date(_year, _month, _day)
            except Exception as ie:
                message += u'<ul>Ngày không đúng định dạng ở ô (' + str(r) + ', 4).</ul>'
        except Exception as e:
            print e
            return {'error': u'File tải lên không phải file Excel'}, message

        try:
            less = sub.lesson_set.get(index = index)
        except Exception as e:
            less = Lesson()
            less.index = index
            less.subject_id = sub
            sub.number_lesson = sub.number_lesson + 1
            sub.save()
